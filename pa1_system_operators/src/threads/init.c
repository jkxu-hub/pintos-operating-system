...
main (void)
{
  ...
  thread_init ();
  console_init ();  
//---At this point main thread is created and running, not on ready_list
  /* Greet user. */
  ...
  serial_init_queue ();
  timer_calibrate ();
//---at this point idle thread is created and blocked
#ifdef FILESYS
...
//---at this point there are 2 thread: main:running idle:blocked  
